    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {  
        ...
        if (empty($_POST['ID'])) {  
            $un = mysql_real_escape_string($connect, $_POST['ID']);  
        } else {  
            $un = FALSE;  
    ...
    if ($un && $p) {  
        ...
        $sq = "SELECT ID, FirstName, LastName, Specialization, Password FROM Doktor WHERE (ID = '$un' AND Password = '$p')";  
    ...
    <input id="ID" type="text" name="ID" size="6" maxlength="6" 
